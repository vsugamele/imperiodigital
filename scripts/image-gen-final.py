#!/usr/bin/env python3
"""
Gera imagem + salva local + sobe pro Google Drive (com Service Account)
"""
import sys
import os
import json
...
def upload_to_drive(filepath, filename, folder_id):
    """Faz upload do arquivo pro Google Drive usando Service Account"""
    try:
...
def generate_and_upload(prompt, folder_id):
    ...
    try:
        ...
        drive_result = upload_to_drive(filepath, filename, folder_id)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Uso: image-gen-final.py 'prompt' [folder_id]")
        sys.exit(1)
