...
function scheduleVideo({ user, videoPath, title, scheduled_date, timezone }) {
  const args = [
...
function schedulePhotos({ user, title, photoPaths, scheduled_date, timezone }) {
  const args = [
...
function main() {
  ...
  for (const item of schedule) {
    ...
    try {
      let resp;
      if (item.kind === 'reels') {
        resp = scheduleVideo({ user, videoPath: reelPath, title: item.title, scheduled_date, timezone: tz });
      } else if (item.kind === 'image') {
        resp = schedulePhotos({ user, title: item.title, photoPaths: [imgPath], scheduled_date, timezone: tz });
      } else {
        resp = schedulePhotos({ user, title: item.title, photoPaths: slidePaths, scheduled_date, timezone: tz });
      }
